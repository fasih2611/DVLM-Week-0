...
def part_3():
    ...
    def get_all_features(dataset, model):
      all_image_features = []
      all_text_features = []
      all_labels = []
      ...
      with torch.no_grad():
          ...
          for image, label in tqdm(dataset, desc=" feat extraction"):
              ...
              text_tokens = clip.tokenize([prompt]).to(device) # Must be a list
      ...
      X = np.vstack(all_image_features)
      Y = np.vstack(all_text_features)
    ...
    print(f"\nZero-shot Accuracy: {acc_baseline:.4f}%")
    ...
    plt.tight_layout()
    plt.savefig("task 4, transform alighnment.png")
    plt.show()
